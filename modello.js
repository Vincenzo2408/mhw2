...
function okay(event){
  const titolo = event.currentTarget.parentNode.querySelector('h1'); /*salvataggio di h1, img e a di quelli cliccati*/
  ...
  if(piacere.textContent === "Aggiungi ai preferiti"){
    ...
    new_div.appendChild(new_h1); /*piazzo dentro il div nuovo creato l'h1, l'img, e a */
    ...
    /*Implementazione dell'evento "rimuovi dai preferiti" a tutti i #mipiace a*/
  ...
  if(piacere.textContent === "Rimuovi dai preferiti"){
    ...
    for(let i=0; i<objectLenght();i++){
      if(list[i].textContent===titolo.textContent){ /*confronta il nome di h1 di quello cliccato dei preferiti con tutti gli h1 delle normali cabine*/
        const cambiopiacere = document.querySelectorAll("#descrizione button") /*appena trovata una corrispondenza mi salvo in cambiopiacere la lista di tutte le a*/
...
function addElement(){
  ...
  for(let i=0; i<objectLenght(); i++){
    ...
    container[i].appendChild(new_h1);
    container[i].appendChild(new_img);
    container[i].appendChild(contenuto);
    container[i].appendChild(new_p);
    container[i].appendChild(new_a);
